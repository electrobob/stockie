...
<html>
 <head>
  <title>CHECK item by name</title>
 </head>
 <body>
<?php
...
} else
{
	...
	$num_rows = mysqli_num_rows($result);
	if($num_rows > 0){
	echo "Requested item checked<br>";
	$row = mysqli_fetch_array($result); 
	  echo $row['id'] . $row['name'];
	  echo "<br>";
	
	mysqli_query($con,"UPDATE " . $user_name . " SET value=1 WHERE name = '" . $item_name_req . "'");
